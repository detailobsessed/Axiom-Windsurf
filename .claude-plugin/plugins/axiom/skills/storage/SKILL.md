...
- SwiftData implementation details (use `swiftdata` skill)
- SQLite/GRDB specifics (use `sqlitedata` or `grdb` skills)
- CloudKit sync implementation (use `cloudkit-ref` skill)
- File protection APIs (use `file-protection-ref` skill)

**Related Skills**:
- Existing database skills: `swiftdata`, `sqlitedata`, `grdb`
- New file skills: `file-protection-ref`, `storage-management-ref`, `storage-diag`
- New cloud skills: `cloudkit-ref`, `icloud-drive-ref`, `cloud-sync-diag`
...
- **Use skill**: `swiftdata` for implementation details
...
- **Use skill**: `sqlitedata` for modern SQLite patterns

**Use GRDB when**:
- Need reactive queries (ValueObservation)
- Complex database operations
- Type-safe query builders
- **Use skill**: `grdb` for advanced patterns
...
1. **SwiftData + CloudKit** (Recommended, iOS 17+):
   ...
   - Easiest approach
   - **Use skill**: `swiftdata` for details
