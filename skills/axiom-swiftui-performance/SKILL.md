...
**For view update debugging:** Use `swiftui-debugging` skill when views aren't updating at all (not slow, just not updating).

**For general profiling:** Use `performance-profiling` skill for non-SwiftUI performance issues (memory, CPU, network).
