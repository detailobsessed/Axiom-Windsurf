...
**For SwiftUI-specific profiling:** See `swiftui-performance` skill for the new SwiftUI Instrument in Instruments 26.

**For memory leaks specifically:** See `memory-debugging` skill for retain cycle diagnosis.

## Related

- `swiftui-performance` skill — SwiftUI Instrument for view update analysis
- `memory-debugging` skill — Retain cycle and leak diagnosis
- `xcode-debugging` skill — Environment-first diagnostics before profiling
- `swift-concurrency` skill — Background thread patterns for CPU work
